...
let rec eval_expr : expr -> exp_val ea_result = fun e ->
  ...
  | SetField(e1,id,e2) ->
    ...
    | Some(true, RefVal i) ->
      eval_expr e2 >>= 
      Store.set_ref g_store i >>= fun _ ->
      return UnitVal
    | Some(true, _) -> error "Field should have refval"
    | Some(false, _z) -> error "Field not mutable") 
